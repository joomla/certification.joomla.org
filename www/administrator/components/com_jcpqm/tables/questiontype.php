...
/**
 * Questiontypes Table class
 */
class JcpqmTableQuestiontype extends JTable
{
	/**
	...
	{
		parent::__construct('#__jcpqm_questiontype', 'id', $db);

		// Adding History Options
		JTableObserverContenthistory::createObserver($this, array('typeAlias' => 'com_jcpqm.questiontype'));
	}	
	...
	/**
	 * Overload the store method for the Questiontype table.
	 *
	...
	public function store($updateNulls = false)
	{
		...
		{
			// New questiontype. A questiontype created and created_by field can be set by the user,
		...
		if (isset($this->alias))
		{
			...
			$table = JTable::getInstance('questiontype', 'JcpqmTable');

			if ($table->load(array('alias' => $this->alias)) && ($table->id != $this->id || $this->id == 0))
			{
				$this->setError(JText::_('COM_JCPQM_QUESTIONTYPE_ERROR_UNIQUE_ALIAS'));
				return false;
			}
		}
	...
	public function check()
	{
		...
		if (!$this->getRules())
		{
			$rules = $this->getDefaultAssetValues('com_jcpqm.questiontype.'.$this->id);
			$this->setRules($rules);
		}
	...
	{
		$k = $this->_tbl_key;
		return 'com_jcpqm.questiontype.'.(int) $this->$k;
	}
